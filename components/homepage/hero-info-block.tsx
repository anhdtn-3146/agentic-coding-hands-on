 ...
 */
const ZERO_COUNTDOWN: CountdownUnit[] = ["DAYS", "HOURS", "MINUTES"].map(
  (label) => ({
    label,
    value: "00",
  }),
...
export default function HeroInfoBlock() {
  ...
  const { countdown, showComingSoon } = useSyncExternalStore(
    subscribe,
    getSnapshot,
    getServerSnapshot,
  );
      ...
      <div className="flex flex-col items-start gap-4">
        ...
        {showComingSoon && (
          <p className="text-[24px] leading-[32px] font-bold">
            {t("home:hero.countdown.comingSoon")}
          </p>
        )}
        ...
        <div className="flex flex-wrap items-center gap-10">
          {countdown.map((unit) => (
            <div key={unit.label} className="flex flex-col items-start gap-3.5">
              ...
              <p className="text-[24px] leading-[32px] font-bold">
                {t(COUNTDOWN_LABEL_KEYS[unit.label])}
              </p>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col items-start gap-2">
        ...
        <div className="flex flex-wrap items-center gap-[60px]">
          {EVENT_INFO.map((info) => (
            <p
              key={info.labelKey}
              className="flex items-center gap-1 text-[24px] leading-[32px] font-bold text-[#FFEA9E]"
            >
              <span className="text-[16px] leading-[24px] tracking-[0.15px] text-white">
                {t(info.labelKey)}
              </span>{" "}
              {t(info.valueKey)}
            </p>
        ...
        {/* mm:2167:9061 */}
        <p className="text-[16px] leading-[24px] font-bold tracking-[0.5px]">
          {t("home:hero.eventInfo.livestreamNote")}
        </p>
      </div>
    </div>
  );
