  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="flex cursor-pointer items-center gap-1 rounded px-3 py-3 text-white transition-colors duration-200 hover:bg-white/10"
      >
        <span className="flex items-center gap-1">
          ...
          <span className="font-(family-name:--font-montserrat) text-base font-bold leading-6 tracking-[0.15px]">
            {current.label}
          </span>
        </span>
        <CustomSvgIcon
      ...
      {open && (
        <ul
          role="listbox"
          aria-label="Language"
          className="absolute right-0 top-full z-30 mt-1 w-28 overflow-hidden rounded-md bg-[#101417] shadow-[0_4px_12px_rgba(0,0,0,0.4)]"
        >
          {LANGUAGES.map((lang) => (
            <li key={lang.code}>
              <button
