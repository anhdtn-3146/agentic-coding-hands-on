...
function CountdownUnit({ value, label }: { value: number; label: string }) {
  ...
  return (
    ...
    <div className="flex flex-col items-start gap-[21px]">
      ...
      <span className="font-(family-name:--font-montserrat) text-[36px] leading-[48px] font-bold text-white">
        {label}
      </span>
    </div>
  );
...
export default function CountdownTimer() {
  ...
  return (
    <div className="flex origin-center scale-50 flex-col items-center gap-6 sm:scale-75 md:scale-100">
      {/* mm:2268:35137 title */}
      <p className="font-(family-name:--font-montserrat) text-center text-[36px] leading-[48px] font-bold text-white">
        {t("countdown:title")}
      </p>
