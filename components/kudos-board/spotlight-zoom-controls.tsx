  ...
  return (
    <div className="absolute right-7 bottom-8 z-20">
      {isOpen && (
        <div className="absolute right-0 bottom-full mb-2 flex items-center gap-1 rounded-full border border-[#998C5F]/70 bg-[#00101A] px-2 py-1.5 shadow-[0_4px_4px_0_rgba(0,0,0,0.25)]">
          <button
            ...
            onClick={onZoomOut}
            className="h-6 w-6 rounded-full text-sm font-bold text-white hover:text-[#FFEA9E]"
          >
            −
          </button>
      ...
      <button
        ...
        onClick={onToggle}
        className="flex h-7.5 w-7.5 items-center justify-center rounded-full border border-white/20 bg-white/10 transition-colors hover:bg-white/20"
      >
        <CustomSvgIcon src="/kudos/icons/pan-zoom.svg" className="h-4 w-4 text-white" />
      </button>
    </div>
  );
}
