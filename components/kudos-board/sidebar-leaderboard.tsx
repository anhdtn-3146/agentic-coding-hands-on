...
import type { LeaderboardEntry } from "@/components/kudos-board/feed-mock-data";
...
export default function SidebarLeaderboard({ title, entries, emptyLabel }: SidebarLeaderboardProps) {
  return (
    <div className="flex w-full flex-col items-center gap-4 rounded-[17px] border border-[#998C5F] bg-[#00070C] py-6 pr-4 pl-6 font-(family-name:--font-montserrat)">
      ...
      <h3 className="w-full text-center text-[22px] leading-7 font-bold whitespace-pre-line text-[#FFEA9E]">
        {title}
      </h3>
      ...
      {entries.length === 0 ? (
        ...
        <ul className="flex w-full flex-col items-start gap-4">
          {entries.map((entry, index) => (
            ...
            >
              <span className="relative block h-16 w-16 shrink-0 overflow-hidden rounded-full border-[1.869px] border-white">
                <Image src={entry.avatar} alt="" fill className="object-cover" sizes="64px" />
              </span>
              <span className="flex min-w-0 flex-col items-start gap-0.5">
                <span className="w-full truncate text-[22px] leading-7 font-bold text-[#FFEA9E]">
                  {entry.name}
                </span>
                <span className="w-full truncate text-base font-bold text-white">
                  {entry.description}
                </span>
              </span>
            </li>
