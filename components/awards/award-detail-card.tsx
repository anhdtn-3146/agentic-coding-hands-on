...
export interface AwardPrize {
  /** e.g. "7.000.000 VNĐ" */
  amount: string;
  /** Optional qualifier below the amount, e.g. "cho mỗi giải thưởng". */
  note?: string;
}
...
export default function AwardDetailCard({
  ...
  return (
    ...
    <div
      className={`flex w-full items-start gap-10 ${reverse ? "flex-row-reverse" : "flex-row"}`}
    >
      ...
      <div className="font-(family-name:--font-montserrat) flex w-120 flex-col gap-8 rounded-2xl backdrop-blur-3xl">
        {/* mm:214:2527 content */}
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="flex items-center gap-2">
            ...
            <span className="text-sm leading-5 font-bold tracking-[0.1px] text-white max-w-15">
              {quantityUnit}
            </span>
          </span>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {prizes.map((prize, index) => (
            <div key={prize.amount} className="flex flex-col gap-4">
              {index > 0 && (
                // mm:313:8498 Frame 524 — "Hoặc" separator (Signature 2025 only)
